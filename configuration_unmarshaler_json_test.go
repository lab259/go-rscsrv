...
var _ = g.Describe("ConfigurationUnmarshelerJson", func() {
	g.It("should unmarshal a configuration", func() {
		...
		Expect(rscsrv.DefaultConfigurationUnmarshalerJson.Unmarshal([]byte(`{"name1": "value 1", "name2": 2}`), &dst)).To(BeNil())
	...
	g.It("should fail unmarshaling a malformed JSON", func() {
		var dst UnmarshalingTest
		Expect(rscsrv.DefaultConfigurationUnmarshalerJson.Unmarshal([]byte(`this is not a JSON`), &dst)).NotTo(BeNil())
	})
})
